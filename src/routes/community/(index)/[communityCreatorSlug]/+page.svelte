...
<div class={"grid grid-cols-3 grid-flow-row gap-3"}>
    {#each data.communityCreatorAndVods.vods as vod}
        <a href={`/community/${data.communityCreatorAndVods.slug}/${vod.slug}`} class={`card group card-compact col-span-1 image-full bg-base-200 shadow-xl hover:scale-105 active:scale-100 transition-all overflow-hidden hover:cursor-pointer select-none ${vod.service === "Youtube" ? "border-red-500 border-2" : vod.service === "Kick" ? "border-2 border-green-200" : vod.service === "Rumble" ? "border-2 border-green-500" : ""}`}>
